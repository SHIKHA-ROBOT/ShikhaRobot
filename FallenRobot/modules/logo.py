...
async def lego(event):
    ...
    try:
        ...
        await telethn.send_file(
            event.chat_id, file=fname, caption=f"ʟᴏɢᴏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ 「 ꜱʜɪᴋʜᴀ 𒆜 ʀᴏʙᴏᴛ 」"
        )
        await pesan.delete()
        if os.path.exists(fname):
            os.remove(fname)
    except Exception as e:
        await event.reply(f"ғʟᴏᴏᴅᴡᴀɪᴛ ᴇʀʀᴏʀ, ʀᴇᴩᴏʀᴛ ᴛʜɪs ᴀᴛ @STD_DEEPANSHU")
